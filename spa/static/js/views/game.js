...
export default class extends AbstractView {
  ...
  async loadGameInfo(gameId) {
    ...
    document.getElementById('msg_text').value = 
      `${msg}${document.getElementById('msg_text').value}`;
  ...
  async loadLastRound(gameId) {
    ...
    if (!this.readonly) {
      ...
      srcImgCard = [
        this.mapCardArt(lastRound.playerCards[0]), 
        this.mapCardArt(lastRound.playerCards[1]),
        this.mapCardArt(lastRound.playerCards[2])
      ]; 
    } else {
      srcImgCard = [
        '/static/img/Minicard_back.svg',
        '/static/img/Minicard_back.svg',
        '/static/img/Minicard_back.svg'
      ];
    }
    ...
    if (!this.readonly) {
      ...
      imgCard[0].onclick = async () => { await this.playCard(0); };
      imgCard[1].onclick = async () => { await this.playCard(1); };
      imgCard[2].onclick = async () => { await this.playCard(2); };
    }
  ...
  async check(roundId) {
    ...
    if (!checkData || checkData.lastTurnSeq == this.turnSeq)
      return;
  ...
  async afterLoad() {
    ...
    if (lastMsg) {
      document.getElementById('msg_text').value = 
      `${lastMsg}${document.getElementById('msg_text').value}`;
    }
